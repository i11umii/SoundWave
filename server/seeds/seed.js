...
// Этот скрипт заполняет базу демо-данными
// Создаются: пользователь, артисты, альбомы, треки и плейлисты
...
const seedDatabase = async () => {
  try {
      ...
      if (data.albums && data.albums.length > 0) {
        ...
        for (const albData of data.albums) {
          const newAlbum = await Album.create({
            title: albData.title,
            artist: artist._id,
            coverUrl: albData.coverImage,
            year: albData.releaseYear,
    ...
    for (let day = 0; day < 14; day++) {
      const tracksPerDay = Math.floor(Math.random() * 6) + 4;
